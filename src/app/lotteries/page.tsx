...
export default function LotteriesPage() {
    ...
    const [allRaffleItems, setAllRaffleItems] = useState<
        Array<{
    ...
    useEffect(() => {
        HomeService.getHomePage(1)
            .then((res) => {
                ...
                // Flatten all raffle items from all sections
                const items = raffles.flatMap((section) =>
    ...
    return (
        <main>
            ...
            <div className="container mx-auto px-4 py-8">
                {allRaffleItems.length === 0 ? (
                    <div className="flex items-center justify-center min-h-[40vh]">
                        <p className="text-lg text-[#797979]">No lotteries available</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                        {allRaffleItems.map(({ item, cellType }, index) => {
                            const aspect =
